                  ...
                  <tr key={index} className="bg-white border-b hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center px-6 text-gray-900 whitespace-nowrap">
                        <div>
                          <ProfileCard>
                            <img src={category?.user?.image?.filePath} alt="User" />
                          </ProfileCard>
                        </div>
                        <div className="pl-3">
                          <div className="text-base font-semibold capitalize">{category?.user?.name}</div>
                          <div className="font-normal text-gray-500">{category?.user?.email}</div> {/* Assuming user info is part of category */}
                        </div>
                      </div>
                    </td>
                    {/* <td className="px-6 py-4">{new Date(category?.createdAt).toLocaleDateString()} </td>  */}
                    <td className="px-6 py-4 font-bold">{category?.title}</td>
                    <td className="px-6 py-4 text-center flex items-center justify-end gap-3 mt-1">
                      {/* <NavLink to={`/category/${category._id}`} type="button" className="font-medium text-indigo-500">
                        <TiEyeOutline size={25} />
                      </NavLink> */}
                      <NavLink to={`/category/update/${category._id}`} className="font-medium text-green-500">
                        <CiEdit size={25} />
                      </NavLink>
                      <button onClick={() => handleDelete(category._id)} className="font-medium text-red-500">
